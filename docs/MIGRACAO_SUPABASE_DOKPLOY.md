...
### Alterações do admin demoram para aparecer no site (Cloudflare)
Se o domínio estiver com o proxy da Cloudflare ativo (nuvem laranja), confira também
no painel da Cloudflare — o `kong.yml` já força `Cache-Control: no-store` nas respostas
de `/rest/v1/*`, mas uma **Page Rule ou Cache Rule com "Cache Everything"** configurada
manualmente na Cloudflare pode ignorar esse header e cachear mesmo assim:
1. Cloudflare → **Caching → Cache Rules** (ou Page Rules, se ainda usar o sistema antigo).
2. Garanta que não existe nenhuma regra "Cache Everything" cobrindo `supabase.seu-dominio.com.br/*`
   ou o domínio principal do site.
3. Se precisar de uma regra ali por outro motivo, adicione uma exceção (Bypass Cache) para
   os caminhos `/rest/v1/*` e `/auth/v1/*`.
4. Em **Caching → Configuration**, o "Browser Cache TTL" também pode segurar resposta antiga
   no navegador do visitante — para as chamadas de API isso já é resolvido pelo header do Kong.
